...
namespace eosio
{
	...
	{
		transaction_id_type id;		//传输id
		time_point_sec expires;		//过期时间(s)
	...
	typedef multi_index_container<
		...
		indexed_by<
			ordered_unique<
				...
				composite_key_compare<sha256_less, std::less<uint32_t>>>, //主键,连接id
			ordered_non_unique<
				tag<by_expiry>,
				member<node_transaction_state, fc::time_point_sec, &node_transaction_state::expires>>, //分索引,过期时间
			ordered_non_unique<
				tag<by_block_num>,
				member<node_transaction_state, uint32_t, &node_transaction_state::block_num>>>> //分索引,块号
		node_transaction_index;

	//节点块状态
	struct peer_block_state
	{
		block_id_type id;			//块id sha256
		uint32_t block_num = 0;		//块号
		uint32_t connection_id = 0; //连接id
		bool have_block = false;	//块是否已经接收(true:已收到块， false:只收到通知)
	};
	...
	typedef multi_index_container<
		...
		indexed_by<
			ordered_unique<tag<by_id>,
						   ...
						   composite_key_compare<std::less<uint32_t>, sha256_less>>, //主键，连接id
			ordered_non_unique<tag<by_block_id>,
							   composite_key<peer_block_state,
											 member<peer_block_state, block_id_type, &eosio::peer_block_state::id>,
											 member<peer_block_state, bool, &eosio::peer_block_state::have_block>>,
							   composite_key_compare<sha256_less, std::greater<bool>>>, //分索引，块id
	...
	private:
		...
		mutable std::mutex sync_mtx;			 //同步互斥锁
		uint32_t sync_known_lib_num{0};			 //已知同步的块号
		uint32_t sync_last_requested_num{0};	 //上一次请求同步的块号
		uint32_t sync_next_expected_num{0};		 //下一个可能同步的块号
		uint32_t sync_req_span{0};				 //同步请求间隙
		connection_ptr sync_source;				 //同步源(网络连接)
		std::atomic<stages> sync_state{in_sync}; //同步状态
	...
	private:
		...
		//捕获块验证(先验证是否是最新的块：是->设置最新块 否->不处理) c:连接 num:已知的块号 id:块号
	...
	public:
		...
		// 同步更新指定块(如果指定的块号不是默认下一个要请求的块号，就赋值给它) blk_id:块id, blk_num:块号, blk_applied:?
	...
	{
		mutable std::mutex blk_state_mtx;  //块状态锁
		peer_block_state_index blk_state;  //节点状态表
		mutable std::mutex local_txns_mtx; //本地传输锁
		node_transaction_index local_txns; //本地传输表
	...
	public:
		...
		//添加节点块 blkid:块id connection_id:连接id
		bool add_peer_block(const block_id_type &blkid, uint32_t connection_id);
		//节点是否有块 blkid:块id, connection_id:连接id
		...
		//传输块过期(删掉过期时间在[0,现在]的块和块号在[1,lib_num]之间的块) lib_num:块号
	...
	{
	public:
		unique_ptr<tcp::acceptor> acceptor;										//监听器
		std::atomic<uint32_t> current_connection_id{0};							//当前连接id
		unique_ptr<sync_manager> sync_master;									//同步管理器
		unique_ptr<dispatch_manager> dispatcher;								//调度器
		string p2p_address;														//p2p地址
		string p2p_server_address;												//p2p服务地址
		vector<string> supplied_peers;											//供应的节点集合
		vector<chain::public_key_type> allowed_peers;							//允许连接的节点集合
		std::map<chain::public_key_type, chain::private_key_type> private_keys; //私钥集合
		...
		possible_connections allowed_connections{None};									  //允许连接的连接集合
		boost::asio::steady_timer::duration connector_period{0};						  //连接超时
		boost::asio::steady_timer::duration txn_exp_period{0};							  //传输超时
		boost::asio::steady_timer::duration resp_expected_period{0};					  //响应超时
		boost::asio::steady_timer::duration keepalive_interval{std::chrono::seconds{32}}; //保活频率

		int max_cleanup_time_ms = 0;		 //最大清空时间(ms)
		uint32_t max_client_count = 0;		 //最大访客数量
		uint32_t max_nodes_per_host = 1;	 //最大节点地址数量(1:1个节点只能有一个地址)
		bool p2p_accept_transactions = true; //是否接受p2p传输

		// 节点验证超时(s)
		const std::chrono::system_clock::duration peer_authentication_interval{std::chrono::seconds{1}};

		chain_id_type chain_id; //链id
		fc::sha256 node_id;		//本机节点id
		string user_agent_name; //用代理人名字

		chain_plugin *chain_plug = nullptr;		  //链插件
		producer_plugin *producer_plug = nullptr; //生产者插件
		bool use_socket_read_watermark = false;	  //是否使用套接字读水印

		mutable std::shared_mutex connections_mtx; //连接共享指针的互斥量
		std::set<connection_ptr> connections;	   //连接集合

		std::mutex connector_check_timer_mtx;						 //连接检查计时器互斥量
		unique_ptr<boost::asio::steady_timer> connector_check_timer; //连接检查计时器
		int connector_checks_in_flight{0};							 //连接检查计时器数量

		std::mutex expire_timer_mtx;						//超时计时器互斥量
		unique_ptr<boost::asio::steady_timer> expire_timer; //超时计时器

		std::mutex keepalive_timer_mtx;						   //保活计时器互斥量
		unique_ptr<boost::asio::steady_timer> keepalive_timer; //保活计时器

		std::atomic<bool> in_shutdown{false}; //net插件是否关闭

		compat::channels::transaction_ack::channel_type::handle incoming_transaction_ack_subscription; //??

		uint16_t thread_pool_size = 2;						   //线程池容量
		optional<eosio::chain::named_thread_pool> thread_pool; //线程池

	private:
		mutable std::mutex chain_info_mtx;	  //链信息互斥量
		uint32_t chain_lib_num{0};			  //??
		uint32_t chain_head_blk_num{0};		  //链头块号
		uint32_t chain_fork_head_blk_num{0};  //链分叉头块号
		block_id_type chain_lib_id;			  //??
		block_id_type chain_head_blk_id;	  //链头块id
		block_id_type chain_fork_head_blk_id; //链分叉头块id
	...
	const fc::string logger_name("net_plugin_impl"); //日志打印器名字
	fc::logger logger;								 //日志打印器
	std::string peer_log_format;					 //节点日志格式化字符串
...
#define peer_elog(PEER, FORMAT, ...)                                                                          \
	...
	constexpr auto def_send_buffer_size = 1024 * 1024 * def_send_buffer_size_mb; //发送缓冲区大小(4mb)
	constexpr auto def_max_write_queue_size = def_send_buffer_size * 10;		 //写队列总容量(4*10mb)
	constexpr auto def_max_trx_in_progress_size = 100 * 1024 * 1024;			 //最大处理同时传输数量（100 MB）
	constexpr auto def_max_consecutive_rejected_blocks = 13;					 //最大拒绝的块,超过此数目就断线(13)
	constexpr auto def_max_consecutive_immediate_connection_close = 9;			 //断线重连次数(9)
	constexpr auto def_max_clients = 25;										 //最大客户端数目,0:无限制(25)
	constexpr auto def_max_nodes_per_host = 1;									 //每个主机最大的节点数(1)
	constexpr auto def_conn_retry_wait = 30;									 //连接重连等待超时(30)
	constexpr auto def_txn_expire_wait = std::chrono::seconds(3);				 //传输超时(3s)
	constexpr auto def_resp_expected_wait = std::chrono::seconds(5);			 //响应超时(5s)
	constexpr auto def_sync_fetch_span = 100;									 //?

	constexpr auto message_header_size = 4;			 //消息头尺寸(4)
	constexpr uint32_t signed_block_which = 7;		 //已签名块消息
	constexpr uint32_t packed_transaction_which = 8; //打包的传输

	constexpr uint16_t net_version_base = 0x04b5;		  //网络基础版本
	constexpr uint16_t net_version_range = 106;			  //网络范围版本
	constexpr uint16_t proto_base = 0;					  //??
	constexpr uint16_t proto_explicit_sync = 1;			  //??
	constexpr uint16_t block_id_notify = 2;				  //??
	constexpr uint16_t net_version = proto_explicit_sync; //网络版本

	//节点同步状态类
	struct peer_sync_state
	{
		//start:开始块号, end:结束块号, last_acted:最后一个活跃的块号
		...
		uint32_t end_block;	   //结束块号
		uint32_t last;		   //上一个发送or接收的块号
	...
	public:
		...
		//添加写缓冲区; buff:缓冲区 callback:回调 to_sync_queue:是否同步写入
	...
	private:
		struct queued_write //写队列
		{
			std::shared_ptr<vector<char>> buff;									  //缓冲区
			std::function<void(boost::system::error_code, std::size_t)> callback; //回调
		};

		mutable std::mutex _mtx;			   //互斥量
		uint32_t _write_queue_size{0};		   //写队列大小
		deque<queued_write> _write_queue;	   //写入队列
		deque<queued_write> _sync_write_queue; //同步写队列,最先调用
		deque<queued_write> _out_queue;		   //输出队列
	};
	...
	public:
		...
		bool start_session();
		//套接字是否打开(线程安全)
		bool socket_is_open() const { return socket_open.load(); }
		//返回节点地址(线程安全)
		const string &peer_address() const { return peer_addr; }
		//设置连接类型
	...
	private:
		static const string unknown;			  //未知字符串
		void update_endpoints();				  //更新端点
		optional<peer_sync_state> peer_requested; //节点同步状态，允许返回无效值
		std::atomic<bool> socket_open{false};	  //套接字是否开启
		const string peer_addr;					  //节点地址
		//连接类型
		enum connection_types : char
		{
			both,			   //2者都是
			transactions_only, //只用来传输
			blocks_only		   //只用来传块
		};
		std::atomic<connection_types> connection_type{both}; //连接类型（默认可以用来传输和传块）

	public:
		boost::asio::io_context::strand strand;							  //顺序调度器（类似于管道）
		std::shared_ptr<tcp::socket> socket;							  //套接字
		fc::message_buffer<1024 * 1024> pending_message_buffer;			  //消息缓冲区(1mb)
		std::atomic<std::size_t> outstanding_read_bytes{0};				  //未读的字节数
		queued_buffer buffer_queue;										  //缓冲区队列
		std::atomic<uint32_t> trx_in_progress_size{0};					  //传输数据数量
		const uint32_t connection_id;									  //连接id
		int16_t sent_handshake_count = 0;								  //发送握手消息数量
		std::atomic<bool> connecting{true};								  //是否正在连接
		std::atomic<bool> syncing{false};								  //是否正在同步发送消息中
		uint16_t protocol_version = 0;									  //协议版本
		uint16_t consecutive_rejected_blocks = 0;						  //连续被拒绝的块数量
		std::atomic<uint16_t> consecutive_immediate_connection_close = 0; //重连次数
		std::mutex response_expected_timer_mtx;							  //返回超时互斥量
		boost::asio::steady_timer response_expected_timer;				  //返回超时计时器
		std::atomic<go_away_reason> no_retry{no_reason};				  //断线理由
		mutable std::mutex conn_mtx;									  //连接互斥量
		optional<request_message> last_req;								  //上一次请求消息
		handshake_message last_handshake_recv;							  //上一次握手收到的消息
		handshake_message last_handshake_sent;							  //上一次握手发送的消息
		block_id_type fork_head;										  //分叉出来的头块
		uint32_t fork_head_num{0};										  //分叉出来的头块号
		fc::time_point last_close;										  //上一次关闭时间
		fc::sha256 conn_node_id;										  //当前握手的连接节点id
		string remote_endpoint_ip;										  //远程端点ip
		string remote_endpoint_port;									  //远程端点端口
		string local_endpoint_ip;										  //本地端点ip
		string local_endpoint_port;										  //本地端点端口
		...
		//是否已连接且不在同步中
		bool current();
		//关闭连接 reconnect:是否立即重连 shutdown:是否立即关闭
		void close(bool reconnect = true, bool shutdown = false);

	private:
		//关闭连接 self:连接, reconnect:是否重连, shutdown:是否关闭
		static void _close(connection *self, bool reconnect, bool shutdown);

	public:
		//普通的握手
		bool populate_handshake(handshake_message &hello, bool force);
		//处理断线 (no_reason|wrong_version 重连)
		bool resolve_and_connect();
		//建立连接(no_reason|wrong_version 重连) resolver:处理器 endpoints:端点
		void connect(const std::shared_ptr<tcp::resolver> &resolver, tcp::resolver::results_type endpoints);
		//开始接收消息（异步循环; 如果缓冲区不够会扩容）
		void start_read_message();
		/**
	 	  * 处理下一个消息 
	 	  * message_length:消息长度
		  * 1.如果消息是已签名的;判断下:块存不存在,能否从节点同步,是否有签名；不通过就关连接
	 	  * 2.如果是打包好的消息;判断下:是否接受p2p传输;不接受就关连接
	 	  * 3.如果以上都不是:关连接;
	 	*/
		...
		//块分叉 msg_head_id:消息头id
		void blk_send_branch(const block_id_type &msg_head_id);
		//分叉实现 msg_head_num:消息头块号, lib_num:块号, head_num:头块号
		...
		//块入列 sb:已签名的块 to_sync_queue:是否写到同步队列
		void enqueue_block(const signed_block_ptr &sb, bool to_sync_queue = false);
		//缓冲区入队 send_buffer:发送缓冲区, close_after_send:断线理由, to_sync_queue:是否发到同步队列
		...
		//接收超时(错误码为空就重连) ec:错误码
		void fetch_timeout(boost::system::error_code ec);
		//发送数据 buff:缓冲区, callback:回调, to_sync_queue:是否写到同步队列
		...
		fc::variant_object get_logger_variant()
		{
			...
			mvo("_id", conn_node_id)("_sid", conn_node_id.str().substr(0, 7))("_ip", remote_endpoint_ip)("_port", remote_endpoint_port)("_lip", local_endpoint_ip)("_lport", local_endpoint_port);
			return mvo;
		}
	};
	...
	void for_each_block_connection(Function f)
	{
		...
		for (auto &c : my_impl->connections)
		{
			if (c->is_transactions_only_connection())
				continue;
			if (!f(c))
				return;
		}
	...
	//设置节点地址类型(传输地址/块地址) peer_add:节点地址
	void connection::set_connection_type(const string &peer_add)
	{
		...
		string::size_type end = colon2 == string::npos
									? string::npos
									: peer_add.find_first_of(" :+=.,<>!$%^&(*)|-#@\t", colon2 + 1); // future proof by including most symbols without using regex
		string host = peer_add.substr(0, colon);
		string port = peer_add.substr(colon + 1, colon2 == string::npos ? string::npos : colon2 - (colon + 1));
		string type = colon2 == string::npos ? "" : end == string::npos ? peer_add.substr(colon2 + 1) : peer_add.substr(colon2 + 1, end - (colon2 + 1));
	...
	void connection::blk_send_branch(const block_id_type &msg_head_id)
	{
		...
		block_id_type lib_id = last_handshake_recv.last_irreversible_block_id; //上一个固定块id
		g_conn.unlock();
		const auto lib_num = block_header::num_from_id(lib_id);
		if (lib_num == 0)
			return; // if last_irreversible_block_id is null (we have not received handshake or reset)
	...
	void connection::cancel_sync(go_away_reason reason)
	{
		...
		default:
			...
			enqueue((sync_request_message){0, 0}); //同步请求消息[开始块id=0, 结束块id=0]
		}
	}
	...
	bool connection::enqueue_sync_block()
	{
		...
		uint32_t num = ++peer_requested->last; //当前块号=上一个请求的块号+1
		if (num == peer_requested->end_block)  //当前块号=结束块号
		{
			peer_requested.reset();
	...
	{
		sync_request_message srm = {start, end}; //请求块消息
	...
	sync_manager::sync_manager(uint32_t req_span)
		: sync_known_lib_num(0), sync_last_requested_num(0), sync_next_expected_num(1), sync_req_span(req_span), sync_source(), sync_state(in_sync)
	{
	}
	...
	//块号是否已经同步了 fork_head_block_num:分叉头块号
	bool sync_manager::is_sync_required(uint32_t fork_head_block_num)
	{
		fc_dlog(logger, "last req = ${req}, last recv = ${recv} known = ${known} our head = ${head}",
				("req", sync_last_requested_num)("recv", sync_next_expected_num)("known", sync_known_lib_num)("head", fork_head_block_num));

		return (sync_last_requested_num < sync_known_lib_num ||
				fork_head_block_num < sync_last_requested_num);
	}

	//开始同步 c:连接, target:目标块号
	void sync_manager::start_sync(const connection_ptr &c, uint32_t target)
	{
		...
		request_next_chunk(std::move(g_sync), c); //请求下一个块
	}

	//再次同步捕获 c:连接, reason:断线理由
	...
	void sync_manager::recv_handshake(const connection_ptr &c, const handshake_message &msg)
	{
		...
		if (head_id == msg.head_id)
		{
			fc_ilog(logger, "handshake from ${ep}, lib ${lib}, head ${head}, head id ${id}.. sync 0",
					("ep", c->peer_name())("lib", msg.last_irreversible_block_num)("head", msg.head_num)("id", msg.head_id.str().substr(8, 16)));
			c->syncing = false;
			notice_message note;
		...
		{
			{
				...
				fc_ilog(logger, "catch_up while in ${s}, fork head num = ${fhn} "
						...
						("s", stage_str(sync_state))("fhn", num)("lib", sync_known_lib_num)("ne", sync_next_expected_num)("id", id.str().substr(8, 16))("p", c->peer_name()));
			}
			uint32_t lib;		   //固定下的块号
			block_id_type head_id; //分叉的头块id
			//拆包； 固定下的块号, 头块号(忽略), 分叉的头块号(忽略)， 固定下的块id(忽略), 头块id(忽略), 分叉的头块id
			...
			req.req_blocks.ids.emplace_back(head_id); //捕获到的头块id放入
		}
		else
		{
			fc_ilog(logger, "none notice while in ${s}, fork head num = ${fhn}, id ${id}..., peer ${p}",
					("s", stage_str(sync_state))("fhn", num)("id", id.str().substr(8, 16))("p", c->peer_name()));
			std::lock_guard<std::mutex> g_conn(c->conn_mtx);
	...
	void sync_manager::sync_recv_notice(const connection_ptr &c, const notice_message &msg)
	{
		...
		{ //如果已经捕获
			...
			else
			{
				...
				{													 //如果块号不在本地
					verify_catchup(c, msg.known_blocks.pending, id); //验证它
				}
				else
				{
					// we already have the block, so update peer with our view of the world
					c->send_handshake(); //已经有了，更新节点
				}
			}
		}
	...
	//更新同步块号 c:连接, blk_id:块id, blk_num:块号, blk_applied:?
	...
	//------------------------------------------------------------------------
	//给节点添加块 blkid:块id connection_id:连接id
	bool dispatch_manager::add_peer_block(const block_id_type &blkid, uint32_t connection_id)
	{
		...
		else if (!bptr->have_block) //如果存在就修改
		{
	...
	{
		std::lock_guard<std::mutex> g(blk_state_mtx);
		const auto &index = blk_state.get<by_block_id>(); //节点状态表 分索引:块id
	...
	void dispatch_manager::expire_txns(uint32_t lib_num)
	{
		...
		old.erase(ex_lo, ex_up); //删掉[0:now()]的传输块
		g.unlock();
	...
	void dispatch_manager::bcast_block(const signed_block_ptr &b, const block_id_type &id)
	{
		...
		for_each_block_connection([this, &id, bnum = b->block_num(), &send_buffer](auto &cp) {
			...
			cp->strand.post([this, cp, id, bnum, send_buffer]() {
				...
				if (!has_block)
				{
					if (!add_peer_block(id, cp->connection_id)) //给节点添加块
					{
	...
	//拒绝传输 trx:已打包的传输块, head_blk_num:头块号
	void dispatch_manager::rejected_transaction(const packed_transaction_ptr &trx, uint32_t head_blk_num)
	{
		...
		if (trx->expiration() > fc::time_point::now())
		{ // no need to update blk_num if already expired
			update_txns_block_num(trx->id(), head_blk_num);
		}
	}

	//接收通知消息 c:连接, msg:通知消息, generated:(无效)
	void dispatch_manager::recv_notice(const connection_ptr &c, const notice_message &msg, bool generated)
	{
		...
		if (msg.known_blocks.mode == normal)
		{
			...
			{
				if (msg.known_blocks.pending == 1)
				{ // block id notify of 2.0.0, ignore
					return;
				}
			}
		}
	...
	void dispatch_manager::retry_fetch(const connection_ptr &c)
	{
		...
		block_id_type bid;
		{
			...
			{
				bid = c->last_req->req_blocks.ids.back(); //拿到请求的块
			}
			else
			{
	...
	bool connection::resolve_and_connect()
	{
		...
		{
			auto connector_period_us = std::chrono::duration_cast<std::chrono::microseconds>(my_impl->connector_period); //微秒
		...
		strand.post([c]() {
			...
			string host = c->peer_address().substr(0, colon);																 //拆分ip
			string port = c->peer_address().substr(colon + 1, colon2 == string::npos ? string::npos : colon2 - (colon + 1)); //拆分端口
			idump((host)(port));																							 //?
	...
	void connection::start_read_message()
	{
		try
		{
			...
			minimum_read = minimum_read != 0 ? minimum_read : message_header_size; //设置一次读最小的数据
															   ...
															   [conn = shared_from_this(), socket = socket](boost::system::error_code ec, std::size_t bytes_transferred) {
																   ...
																   try
																   {
																	   if (!ec)
																	   {
																		   ...
																		   while (conn->pending_message_buffer.bytes_to_read() > 0) //直到耗尽读缓冲区
																		   {
																			   uint32_t bytes_in_buffer = conn->pending_message_buffer.bytes_to_read();

																			   if (bytes_in_buffer < message_header_size) //如果连消息头都不够，砍掉它
																			   {
																			   ...
																			   else
																			   {
																				   ...
																				   conn->pending_message_buffer.peek(&message_length, sizeof(message_length), index); //读一下消息长度
																				   if (message_length > def_send_buffer_size * 2 || message_length == 0)			  //消息长度不能超出[0, 8mb]
																				   {
																				   ...
																				   auto total_message_bytes = message_length + message_header_size; //总消息长度=消息实体长度+消息头长度
																				   ...
																				   {
																					   auto outstanding_message_bytes = total_message_bytes - bytes_in_buffer;		//缺失的消息长度
																					   auto available_buffer_bytes = conn->pending_message_buffer.bytes_to_write(); //获得缓冲区还可以写入的容量
																					   if (outstanding_message_bytes > available_buffer_bytes)
																					   {
																						   conn->pending_message_buffer.add_space(outstanding_message_bytes - available_buffer_bytes); //扩容
																					   }
	...
	bool connection::process_next_message(uint32_t message_length)
	{
		try
		{
			...
			if (which == signed_block_which) //如果是已签名的块消息
			{
				block_header bh;
				fc::raw::unpack(peek_ds, bh); //解析消息头

				const block_id_type blk_id = bh.id();
				const uint32_t blk_num = bh.block_num();
				if (my_impl->dispatcher->have_block(blk_id)) //如果块已经存在了
				{
				...
				fc_dlog(logger, "${p} received block ${num}, id ${id}..., latency: ${latency}",
						("p", peer_name())("num", bh.block_num())("id", blk_id.str().substr(8, 16))("latency", (fc::time_point::now() - bh.timestamp).count() / 1000));
				if (!my_impl->sync_master->syncing_with_peer()) //无法同步
				{
					uint32_t lib = 0;
				...
				if (has_webauthn_sig) //收到已签名的块为什么要关连接??
				{
			...
			else if (which == packed_transaction_which) //如果是打包好的消息
			{
	...
	{
		controller &cc = chain_plug->chain();						   //链
		std::lock_guard<std::mutex> g(chain_info_mtx);				   //互斥量
		chain_lib_num = cc.last_irreversible_block_num();			   //固定下的块号
		chain_lib_id = cc.last_irreversible_block_id();				   //固定下的块id
		chain_head_blk_num = cc.head_block_num();					   //头块号
		chain_head_blk_id = cc.head_block_id();						   //头块id
		chain_fork_head_blk_num = cc.fork_db_pending_head_block_num(); //分叉的头块号
		chain_fork_head_blk_id = cc.fork_db_pending_head_block_id();   //分叉的头块id
	...
	//获得: 固定下的块号, 头块号, 分叉的头块号， 固定下的块id, 头块id, 分叉的头块id
	...
	bool connection::is_valid(const handshake_message &msg)
	{
		...
		// affecting state.
		bool valid = true;
		if (msg.last_irreversible_block_num > msg.head_num) //不是最新的块
		{
		...
		if (msg.p2p_address.empty()) //消息的p2p地址为空
		{
			fc_wlog(logger, "Handshake message validation: p2p_address is null string");
			valid = false;
		}
		else if (msg.p2p_address.length() > max_handshake_str_length) //地址超长
		{
	...
	void connection::handle_message(const handshake_message &msg)
	{
		...
		if (msg.generation == 1)
		{
			...
			if (!my_impl->authenticate_peer(msg)) //验证节点
			{
	 ...
	 */
		...
		double offset = (double(rec - org) + double(msg.xmt - dst)) / 2; //偏移时间
		...
		if (msg.known_trx.mode != none)
		{
			if (logger.is_enabled(fc::log_level::debug))
			{
				...
				fc_dlog(logger, "this is a ${m} notice with ${n} pending blocks: ${num} ${id}...",
						("m", modes_str(msg.known_blocks.mode))("n", msg.known_blocks.pending)("num", block_header::num_from_id(blkid))("id", blkid.str().substr(8, 16)));
			}
		}
	...
	void net_plugin_impl::start_conn_timer(boost::asio::steady_timer::duration du, std::weak_ptr<connection> from_connection)
	{
		...
		connector_check_timer->async_wait([my = shared_from_this(), from_connection](boost::system::error_code ec) {
			...
			if (!ec)
			{
				my->connection_monitor(from_connection, num_in_flight == 0); //连接维护器
			}
			else
			{
				if (num_in_flight == 0)
	...
	//连接维护器 from_connection:连接, reschedule:是否重新启动连接计时器
	...
	{
		cfg.add_options()("p2p-listen-endpoint", bpo::value<string>()->default_value("0.0.0.0:9876"), 
			"The actual host:port used to listen for incoming p2p connections.")("p2p-server-address", 
			bpo::value<string>(), 
			...
			("net-threads", bpo::value<uint16_t>()->default_value(my->thread_pool_size), "Number of worker threads in net_plugin thread pool")
